...
- **QA-проверка страниц** — инструмент `product_check`: headless-браузер проверяет веб-страницу на viewport overflow, наложения элементов, сломанные изображения, мёртвые кнопки, обрезанный текст, accessibility-проблемы и JS-ошибки. Весь вывод — текст, Vision-модель не нужна
