/* ================================================================
   ...
   ================================================================ */
    ...
    // Shows inline UI when terminal detects an interactive prompt (y/n, selection, etc.)
    // The model suggests a response; user can approve, edit, or skip.
    function showTerminalInteractivePrompt(output, promptText, promptId, suggestion) {
      // Remove any existing interactive prompt dialog
      const existing = document.querySelector('.terminal-interactive-prompt');
      if (existing) existing.remove();
      ...
      const actionEl = document.createElement('div');
      actionEl.className = 'terminal-interactive-prompt';

      // Show last 3 lines of output as context
      const outputLines = (output || '').trim().split(/\r?\n/).filter(l => l.trim());
      const contextLines = outputLines.slice(-4).map(l => escapeHtml(l)).join('\n');

      actionEl.innerHTML = `
        <div class="terminal-prompt-header">
          <span class="codicon codicon-terminal"></span>
          Терминал ожидает ввод
        </div>
        <div class="terminal-prompt-output"><pre>${contextLines}</pre></div>
        <div class="terminal-prompt-input-row">
          <textarea class="terminal-prompt-input" id="terminal-prompt-input" rows="1" placeholder="Ответ…">${escapeHtml(suggestion !== undefined && suggestion !== null ? suggestion : (promptText || ''))}</textarea>
        </div>
        <div class="terminal-prompt-actions">
          <button class="terminal-confirm-btn allow" id="terminal-prompt-send">Отправить</button>
          <button class="terminal-confirm-btn deny" id="terminal-prompt-skip">Пропустить</button>
        </div>
      `;

      messageContent.appendChild(actionEl);

      const inputArea = actionEl.querySelector('#terminal-prompt-input');
      ...
      // Send response
      actionEl.querySelector('#terminal-prompt-send').addEventListener('click', () => {
        const response = inputArea.value;
        // Allow empty string for Enter-to-continue prompts
        if (response === null || response === undefined) return;
        vscode.postMessage({ type: 'terminalPromptResponse', response, promptId });
        actionEl.innerHTML = `
          <div class="terminal-prompt-header sent">
            <span class="codicon codicon-check"></span>
            Отправлено: <code>${response ? escapeHtml(response.length > 60 ? response.slice(0, 60) + '…' : response) : '⏎ Enter'}</code>
          </div>
        `;
      });

      // Skip — let terminal continue without input
      actionEl.querySelector('#terminal-prompt-skip').addEventListener('click', () => {
        vscode.postMessage({ type: 'terminalPromptResponse', response: null, promptId });
        actionEl.innerHTML = `
          <div class="terminal-prompt-header skipped">
            <span class="codicon codicon-circle-slash"></span>
            Ввод пропущен
          </div>
        `;
    ...
    // ── ask_user: inline question UI ────────────────────────────────────────
    function showUserQuestion(question, options, replyTo) {
      // Find the target assistant message bubble to attach the UI to
      let targetMsg = null;
      if (replyTo) {
        targetMsg = chatContainer.querySelector(`.message.assistant[data-msg-id="${replyTo}"]`);
      }
      if (!targetMsg) {
        const all = chatContainer.querySelectorAll('.message.assistant');
        targetMsg = all.length > 0 ? all[all.length - 1] : null;
      }
      if (!targetMsg) return;
      ...
      const optionsHtml = (options || []).map((opt, i) =>
        `<button class="ask-user-option-btn" data-idx="${i}">${escapeHtml(opt)}</button>`
      ).join('');

      const actionEl = document.createElement('div');
      actionEl.className = 'ask-user-inline';
      actionEl.innerHTML = `
        <div class="ask-user-box">
          <div class="ask-user-label">
            <span class="codicon codicon-comment-discussion"></span>
            <span class="ask-user-question">${escapeHtml(question)}</span>
          </div>
          ...
          <div class="ask-user-input-row">
            <textarea
              ...
              rows="2"
            ></textarea>
            <button class="ask-user-send-btn">
      ...
      const textarea = actionEl.querySelector('.ask-user-textarea');
      const sendBtn = actionEl.querySelector('.ask-user-send-btn');

      // Clicking a preset option fills the textarea
      actionEl.querySelectorAll('.ask-user-option-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          textarea.value = btn.textContent.trim();
          // Visual active state
          actionEl.querySelectorAll('.ask-user-option-btn').forEach(b => b.classList.remove('active'));
      ...
      function submitAnswer() {
        ...
        // Replace the interactive block with a compact "answered" card
        const answeredHtml = `
          <div class="ask-user-answered">
            <span class="codicon codicon-check"></span>
            <span class="ask-user-answered-text">${escapeHtml(answer)}</span>
          </div>
        `;
        actionEl.outerHTML = answeredHtml;

        vscode.postMessage({ type: 'userQuestionResponse', answer });
      }
