...
export class ChatViewProvider implements WebviewViewProvider {
  ...
  // Terminal interactive prompt pending promise
  private pendingTerminalPrompt: {
    resolve: (result: { response: string } | null) => void;
    promptId: string;
  } | null = null;
  // User question pending promise (ask_user tool)
  private pendingUserQuestion: {
    resolve: (answer: string) => void;
    reject: (error: Error) => void;
  } | null = null;
  // Domain handler
  private snapshots: SnapshotHandler;
  // Commit manager (add_commit / get_commit tools)
  ...
  constructor(
    ...
    this.commitManager = new CommitManager(this.storageManager.getSessionsDir());
  ...
  case "attachFile": {
      ...
      case "terminalPromptResponse": {
        // User responded to interactive terminal prompt
        if (this.pendingTerminalPrompt) {
          if (message.response !== null && message.response !== undefined) {
            this.pendingTerminalPrompt.resolve({ response: message.response });
          } else {
            this.pendingTerminalPrompt.resolve(null);
          }
      ...
      case "userQuestionResponse": {
        ...
        if (this.pendingUserQuestion) {
          ...
          this.pendingUserQuestion.resolve(answer || '(no answer)');
  ...
  private async syncSessionsFromDisk() {
    try {
      ...
      if (changed) {
        this.sessions = valid.map((s: any) => ({ id: s.id, title: s.title, date: s.date, mode: s.mode, messages: [] } as ChatSession));
        // Persist cleaned index so disk and UI stay in sync
        (async () => {
          try {
            await this.storageManager.saveSessionsIndex(valid.map(v => ({ id: v.id, title: v.title, date: v.date, mode: v.mode })));
          } catch (err) {
  ...
  public async handleUserMessage(text: string, mode: ChatMode = 'agent') {
    ...
    if (this.history.length === 0) {
      ...
      // Also store mode on in-memory session for history filtering
      const sess = this.sessions.find(s => s.id === this.currentSessionId);
      if (sess) sess.mode = mode;
    ...
    if (mode === 'agent' || mode === 'chat') {
      ...
      try {
        ...
        const handled = await runOpenRouterAgentLoop({
          ...
          requestTerminalInteractiveResponse: this.requestTerminalInteractiveResponse.bind(this),
          requestUserQuestion: this.requestUserQuestion.bind(this),
  ...
  private deleteSession(sessionId: string) {
      ...
      (async () => {
        try {
          ...
          const idx = await this.storageManager.listSessions();
          this.sessions = idx.map((s: any) => ({ id: s.id, title: s.title, date: s.date, mode: s.mode, messages: [] } as ChatSession));
        } catch (err) {
  ...
  /**
   * Request user response to an interactive terminal prompt.
   * Shows prompt UI in chat, resolves with { response } or null (skip).
   */
  private requestTerminalInteractiveResponse(output: string, promptText: string, suggestion: string = ''): Promise<{ response: string } | null> {
    // If auto-run terminal is on — immediately accept the suggestion without showing UI
    const autoRun = workspace.getConfiguration('ashibaltAi').get<boolean>('autoRunTerminal', false);
    if (autoRun) {
      return Promise.resolve({ response: suggestion });
    }

    return new Promise((resolve) => {
      // Cancel previous pending prompt
      if (this.pendingTerminalPrompt) {
        this.pendingTerminalPrompt.resolve(null);
      }
      if (!this.view) {
      ...
      this.pendingTerminalPrompt = { resolve, promptId };

      // Send interactive prompt to WebView UI
      this.postMessage({
        type: 'terminalInteractivePrompt',
        output,
        prompt: promptText,
        suggestion,
        id: promptId
  ...
  private cancelPendingTerminalConfirmation() {
    ...
    if (this.pendingUserQuestion) {
      this.pendingUserQuestion.reject(new Error('Запрос вопроса отменён'));
      this.pendingUserQuestion = null;
    }
  }
  ...
  /**
   * Ask the user a clarifying question inline and wait for their answer.
   * Called by the ask_user tool in agentLoop.ts.
   */
  private requestUserQuestion(question: string, options: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
      if (this.pendingUserQuestion) {
        this.pendingUserQuestion.reject(new Error('New question cancelled previous'));
      }
      if (!this.view) {
        reject(new Error('WebView unavailable'));
        return;
      }
      this.pendingUserQuestion = { resolve, reject };
      // Note: userQuestionRequest is already sent by agentLoop before calling this callback,
      // so we only need to store the promise here.
    });
  }
  ...
  private saveHistory() {
    ...
    const title = userMessages.length > 0 
      ...
      : 'New Chat';
    const sessionMeta = { id: this.currentSessionId, title, date: Date.now(), mode: this.currentMode };

    // Update in-memory sessions list
    const existingSessionIndex = this.sessions.findIndex(s => s.id === this.currentSessionId);
    if (existingSessionIndex !== -1) {
      this.sessions[existingSessionIndex] = { id: sessionMeta.id, title: sessionMeta.title, date: sessionMeta.date, mode: sessionMeta.mode, messages: [] };
    } else {
      this.sessions.unshift({ id: sessionMeta.id, title: sessionMeta.title, date: sessionMeta.date, mode: sessionMeta.mode, messages: [] });
    }
    ...
    this.storageManager.saveSessionsIndex(this.sessions.map(s => ({ id: s.id, title: s.title, date: s.date, mode: s.mode })))
      .catch(err => {
