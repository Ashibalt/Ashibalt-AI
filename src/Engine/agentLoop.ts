...
import { tools as availableTools, executeTool, setToolPostMessage, setInteractivePromptHandler } from './toolCalling';
import { resetTasks, applyTasksInlineUpdate } from './tools/tasksTool';
...
import { CommitManager } from '../Storage/commitManager';
import { executeAddCommit, executeGetCommit } from './tools/commitTool';

// Re-export parseApiError for consumers that import from agentLoop
export { parseApiError } from './agentErrors';

/**
 * Extracts and applies a <tasks>...</tasks> inline JSON block from model text.
 * Returns the content with the tag stripped — it never reaches the API context or the UI.
 *
 * Example model output:
 *   "Done! Here's the result.\n<tasks>{\"done\":[0,1]}</tasks>"
 *
 * Supported payload: done (number[]), update ({index,text?,done?}[]), clear (boolean).
 */
function processInlineTasks(content: string, postMessage: (msg: any) => void): string {
  if (!content.includes('<tasks>')) return content;
  return content.replace(/<tasks>([\s\S]*?)<\/tasks>/g, (_match, json) => {
    try {
      const payload = JSON.parse(json.trim());
      applyTasksInlineUpdate(payload, postMessage);
      logger.log(
        `[AGENT] Inline <tasks> applied — done=${JSON.stringify(payload.done)} update=${JSON.stringify(payload.update)} clear=${payload.clear}`
      );
    } catch (e) {
      logger.log(`[AGENT] Inline <tasks> parse error: ${e}`);
    }
    return '';
  }).trim();
}
...
interface AgentLoopOptions {
  ...
  /** Callback to show interactive terminal prompt to user and get model-suggested response. Returns response text or null to skip. */
  requestTerminalInteractiveResponse?: (output: string, promptText: string, suggestion: string) => Promise<{ response: string } | null>;
  /** Callback to ask the user a clarifying question inline. Pauses the agent loop until the user responds. */
  requestUserQuestion?: (question: string, options: string[]) => Promise<string>;
  /** CommitManager instance for add_commit / get_commit tools. */
...
export async function runOpenRouterAgentLoop(opts: AgentLoopOptions): Promise<boolean> {
  const {
    ...
    requestTerminalInteractiveResponse,
    requestUserQuestion,
    commitManager
  } = opts;

  // Wire postMessage into tools that need UI updates (e.g. tasks panel)
  setToolPostMessage(postMessage);

  // Wire interactive prompt handler for terminal tool
  if (requestTerminalInteractiveResponse) {
    setInteractivePromptHandler(requestTerminalInteractiveResponse);
  }

  // Auto-clear tasks from previous request — new user message = fresh task list
  resetTasks();
  postMessage({ type: 'tasksUpdate', tasks: [] });
...
Use reasoning ONLY for analysis. ALL tool invocations MUST go through the function calling API.
  ...
  while (iteration < maxIterations) {
    ...
    if (!response.tool_calls || response.tool_calls.length === 0) {
      // Strip <tasks>...</tasks> inline update tags before persisting to conversation.
      // The model can mark task steps done via the tag instead of calling tasks() —
      // the tag is processed here and removed so it never enters the context window.
      let finalContent = response.content || '';
      if (finalContent.includes('<tasks>')) {
        finalContent = processInlineTasks(finalContent, postMessage);
        accumulatedContent = finalContent;
        // Keep collectedActions in sync — patch the last text action
        const lastTextIdx = [...collectedActions].map(a => a.type).lastIndexOf('text');
        if (lastTextIdx >= 0) {
          (collectedActions[lastTextIdx] as any).content = finalContent;
        }
        // Push to UI with stripped content
        updateHistoryEntry(assistantPlaceholderId, finalContent, true, collectedActions, effectiveModel);
      }
      ...
      logger.log(`[AGENT] No tool calls, ending loop (text pushed to conversation: ${!!finalContent})`);
      break;
    }
    ...
    for (const toolCall of response.tool_calls) {
      ...
      const TOOL_NAME_REMAP: Record<string, string> = {
        ...
        'lsp_rename': 'lsp',
        'lsp_bridge': 'lsp',
        // product_check variants
        'check_page': 'product_check',
        'page_check': 'product_check',
        'visual_check': 'product_check',
        'ui_check': 'product_check',
        'qa_check': 'product_check',
        'audit_page': 'product_check',
        'check_url': 'product_check',
        'check_website': 'product_check',
        'webpage_check': 'product_check',
        'browser_check': 'product_check',
      };
      ...
      // ── ask_user: pause agent loop until user responds ─────────────────────
      if (toolName === 'ask_user') {
        if (requestUserQuestion) {
          postMessage({ type: 'userQuestionRequest', id: assistantPlaceholderId, question: args.question, options: args.options || [] });
          try {
            const answer = await requestUserQuestion(args.question, args.options || []);
            result = { answer };
          } catch {
            result = { error: 'The question was dismissed.' };
          }
        } else {
          result = { error: 'ask_user is not supported in the current context.' };
        }
      // ── add_commit / get_commit ─────────────────────────────────────────────
      } else if (toolName === 'add_commit' || toolName === 'get_commit') {
        if (commitManager) {
          try {
            if (toolName === 'add_commit') {
              result = await executeAddCommit(args, workspaceRoot || '', currentSessionId, commitManager);
            } else {
              result = await executeGetCommit(args, workspaceRoot || '', currentSessionId, commitManager);
            }
          } catch (err: any) {
            result = { success: false, message: `ERROR: ${err.message || String(err)}` };
          }
        } else {
          result = { success: false, message: 'Commit manager is not available in this context.' };
        }
      }

      // Special handling for terminal run action - may require user confirmation
      if (!result) {
      const terminalAction = (args?.action ?? 'run').toLowerCase();
      const isTerminalRun = (toolName === 'terminal' || toolName === 'run_terminal_command') && (terminalAction === 'run' || terminalAction === '');
      if (isTerminalRun) {
      ...
      } // end terminal/standard tool block
